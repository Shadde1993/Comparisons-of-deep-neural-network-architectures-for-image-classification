...
m = size(V,1);
numOfBatches = floor(m/BatchSize);
...
for iter=1:MaxIter
	...
	for batch=1:BatchSize:num
       ...
       % err = err + sum(sum((vis1-vis0).^2))/BatchSize;
    end
   % disp(['RBM train:epoch ' num2str(iter) '/' num2str(MaxIter) '.Average reconstruction error is:' num2str(err/numOfBatches)]);
	...
	if( Verbose )
        H = v2h( rbm, V );
        bH = double( rand(size(H)) < H );
        Vr = h2v( rbm, bH );
       

        
		err = power( V - Vr, 2 );
		rbm.rmse(iter) = sqrt( sum(err(:)) / numel(err) );
